...
export function AgentVisualizerPanel() {
  ...
  // Loading state check
  const isLoading = false; // Controlled by parent
  const hasNoAgents = agents.length === 0;

  // Seed Agents Button Component
function SeedAgentsButton() {
  ...
  async function seedAgents() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setAgents(agents);
        setMessage(`✅ Seeded ${agents.length} agents!`);
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      >
        {loading ? '⏳ Seeding...' : '🚀 Seed Demo Agents'}
      </button>
      ...
      {hasNoAgents ? (
        ...
        <div className="flex-1 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-400">No agents yet - seed the database to see your AI team!</p>
          </div>
          <SeedAgentsButton />
        </div>
      ) : (
