...
export async function GET() {
  const id = generateId();
  const sql = 'SELECT id, name, slug, description, emoji, color, division, specialization, source, status, capabilities, technical_skills, personality_traits, system_prompt, model_config, metrics FROM agents ORDER BY division, name';
  ...
  const result = await new Promise((resolve) => {
    const req = https.request({
      ...
      res.on('end', () => {
        ...
        catch (e) { resolve({ parseError: true, body: body.substring(0, 200) }); }
      });
    });
